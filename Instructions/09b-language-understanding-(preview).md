...
Un modello di comprensione del linguaggio per un'applicazione orologio potrebbe ad esempio elaborare un input simile al seguente:
...
2. Selezionare il pulsante **&#65291;Crea una risorsa**, cercare *lingua* e creare una risorsa del **servizio Lingua** con le impostazioni seguenti.
    ...
    - **Piano tariffario**: Gratuito (F0) (*se questo piano non è disponibile, selezionare Standard (S)* )
...
1. In una nuova scheda del browser aprire il portale Language Studio all'indirizzo `https://language.azure.com` e accedere usando l'account Microsoft associato alla sottoscrizione di Azure.
...
5. Nella finestra di dialogo **Crea un progetto**, nella pagina **Scegliere un tipo di progetto** selezionare **Conversazione** e fare clic su **Avanti**.
6. Nella pagina **Enter basic information** (Immetti informazioni di base) immettere i dati seguenti e fare clic su **Avanti**:
    - **Nome**: `Clock`
...
1. Nel riquadro a sinistra selezionare la pagina **Esegui training del modello** e selezionare l'opzione per eseguire il training di un nuovo modello, assegnare al modello il nome **Clock** e assicurarsi che l'opzione per eseguire la valutazione durante il training sia selezionata. Fare clic su **Esegui il training** per eseguire il training del modello.
2. Al termine del training (operazione che potrebbe richiedere un certo tempo), visualizzare la pagina **View model details** (Visualizza dettagli modello) e selezionare il modello **Clock**. Visualizzare quindi le metriche di valutazione complessive e per finalità (*precisione*, *richiamo* e *punteggio F1*) e la *matrice di confusione* generata dalla valutazione eseguita durante il training (si noti che a causa del numero ridotto di espressioni di esempio, nei risultati potrebbero non essere incluse tutte le finalità).
...
3. Nella pagina **Distribuisci modello** selezionare il modello **Clock** e distribuirlo. Questa operazione potrebbe richiedere tempo.
4. Dopo la distribuzione del modello, nella pagina **Testa modello** selezionare il modello **Clock**.
5. Immettere il testo seguente e quindi fare clic su **Esegui il test**:
...
6. Deselezionare la casella di testo e quindi eseguire un altro test con il testo seguente:

    `tell me the time`

    Di nuovo, esaminare la finalità prevista e il punteggio di attendibilità.

7. Provare il testo seguente:
...
3. Nella pagina per l'entità **Weekday** fare clic su **&#65291; Aggiungi un nuovo elenco** nella sezione **Elenco**. Immettere quindi il valore e il sinonimo seguenti e fare clic su **Salva**:
...
1. In Language Studio tornare alla pagina **Build schema** (Crea schema) e quindi nella scheda **Entità** selezionare **&#65291; Aggiungi** per aggiungere una nuova entità.
2. Nella finestra di dialogo **Add an entity** (Aggiungi un'entità) immettere il nome di entità **Date** e selezionare il tipo di entità **Predefinito**. Fare quindi clic su **Aggiungi entità**.
...
1. Nella pagina **Esegui training del modello** selezionare l'opzione per sovrascrivere il modello esistente e specificare il modello **Clock**. Assicurarsi che sia selezionata l'opzione per eseguire la valutazione durante il training e fare clic su **Esegui il training** per eseguire il training del modello, confermando di voler sovrascrivere il modello esistente.
2. Al termine del training (operazione che potrebbe richiedere un certo tempo), visualizzare la pagina **View model details** (Visualizza dettagli modello) e selezionare il modello **Clock**. Visualizzare quindi le metriche di valutazione complessive, per entità e per finalità (*precisione*, *richiamo* e *punteggio F1*) e la *matrice di confusione* generata dalla valutazione eseguita durante il training (si noti che a causa del numero ridotto di espressioni di esempio, nei risultati potrebbero non essere incluse tutte le finalità).
3. Nella pagina **Distribuisci modello** selezionare il modello **Clock** e distribuirlo. Questa operazione potrebbe richiedere tempo.
4. Quando l'app viene distribuita, nella pagina **Testa modello** selezionare il modello **Clock** e quindi testarlo con il testo seguente:
...
8. Esaminare la risposta JSON restituita dall'app, che indicherà la finalità con il punteggio più alto prevista per l'input (che dovrebbe essere **GetTime**).
...
1. Nella pagina **Progetti** di Language Studio selezionare il progetto **Clock (Conversazione)** .
